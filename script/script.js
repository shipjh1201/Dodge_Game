...
function createShotgunProj() {
	...
	setTimeout(function () {
		state.launchable = true;
	}, opt.proj.shotgun.attackInterval);
	const angle = Math.atan2(mousePoint.y - player.y, mousePoint.x - player.x);
...
function createGatlingProj() {
	...
	setTimeout(function () {
		state.launchable = true;
	}, opt.proj.gatling.attackInterval);
	const angle = Math.atan2(mousePoint.y - player.y, mousePoint.x - player.x);
	const randAngle = random.arrange(
...
function createBoomerangProj() {
	...
	setTimeout(function () {
		state.launchable = true;
	}, opt.proj.boomerang.attackInterval);
	const projectile = new Boomerang(
		player.x,
		player.y,
		opt.proj.boomerang.radius,
		opt.proj.boomerang.color,
		{
			x: 0,
			y: 0,
		},
		opt.proj.boomerang.dmg,
		opt.proj.boomerang.penetration
	);
...
function createBasicEnemy() {
	...
	enemies.basic.push(
		new Enemy(
			...
			y,
			radius,
			`hsl(${opt.enemy.basic.color}, ${random.arrange(50, 80)}%, ${random.arrange(50, 80)}%)`,
			{
				x:
					(((Math.cos(angle) / radius ** 0.5) *
						(opt.enemy.basic.maxRadius + opt.enemy.basic.minRadius)) /
						2) *
...
function createChaseEnemy() {
	...
	enemies.chase.push(
		new Enemy(
			...
			y,
			radius,
			`hsl(${opt.enemy.chase.color}, ${random.arrange(50, 80)}%, ${random.arrange(50, 80)}%)`,
			{
				x: 0,
				y: 0,
			},
...
function createBounceEnemy() {
	...
	enemies.bounce.push(
		new Enemy(
			...
			y,
			radius,
			`hsl(${opt.enemy.bounce.color}, ${random.arrange(50, 80)}%, ${random.arrange(50, 80)}%)`,
			{
				x:
					(((Math.cos(angle) / radius ** 0.5) *
						(opt.enemy.bounce.maxRadius + opt.enemy.bounce.minRadius)) /
						2) *
...
function mousemoveHandler(e) {
	if (mousePoint.isMouseOverInCanvas === true) {
		...
		mousePoint.x =
			((e.clientX / bodyWidth - (0.5 - canvasSide / (2 * bodyWidth))) * bodyWidth * canvas.width) /
			canvasSide;
		mousePoint.y =
			((e.clientY / bodyHeight - (0.5 - canvasSide / (2 * bodyHeight))) *
				bodyHeight *
				canvas.height) /
			canvasSide;
	}
